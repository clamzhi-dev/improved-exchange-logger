# Improved Exchange Logger

A RuneLite plugin that writes every Grand Exchange transaction to a log file
on disk, in plain text, tabular (CSV), or JSON format. Built on top of the
original [Exchange Logger](https://github.com/istid/exchange-logger) by
istid and Mreedon.
...
```
%USERPROFILE%\.runelite\improved-exchange-logger\exchange.log
```
